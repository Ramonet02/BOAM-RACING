...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   Se lee como un documento de identidad / licencia FIA:
     ...
     │  RETRATO │ PILOTO                          │
     │  (Rally  │ ALEX LORAS                      │
     │   Image) │ INDICATIVO · ORIGEN             │  campos etiquetados en mono
     │   ⊘sello │ 41°23'14.6"N 2°10'07.0"E        │
     ├──────────┴─────────────────────────────────┤
     │ especialidad (solo si ya está escrita)     │
     ├────────────────────────────────────────────┤
     │ REDES · @handle (solo si ya las hay)       │
     ├────────────────────────────────────────────┤
   ...
   · DATOS: todo sale de `src/lib/team.ts` vía props. Aquí no hay ni un
     nombre ni una coordenada escritos a mano. El grupo sanguíneo ya no
     existe: es un dato de salud y no se publica.
   · COPY: cualquier texto de interfaz viene de `useT()`. Los nombres
     propios (persona, apodo del coche, marca) NO se traducen.
   · PENDIENTES: los campos que en `team.ts` son el centinela `PENDING` NO
     se pintan. Antes salían como casillas "por confirmar": 24 en la
     página, y un documento lleno de huecos resta credibilidad (Stanford,
     guía 4). Cada bloque aparece el día que su dato existe. El retrato
     pendiente se queda en las iniciales.
...
import { AtSign, Fingerprint, Link2, MapPin, Music2 } from "lucide-react";
...
const ROLE_TONE: Record<CrewRole, RoleTone> = {
  ...
  copiloto: {
    /* sand-solid: en desert el arena pleno da 3,29:1 a 11 px. */
    accentText: "text-sand-solid",
    accentBar: "bg-sand",
 ...
 * etiquetas largas ("Especialidad") dentro de una tarjeta estrecha sin
...
function PassportField({
  ...
  return (
    <div className="min-w-0 border-b border-slate/70 py-2 last:border-b-0">
      <dt className="telemetry-label flex min-w-0 items-center gap-1.5 text-[0.6875rem] sm:text-[0.6875rem]">
        {icon}
...
export default function PilotPassport({
  ...
  /* Redes: sólo las declaradas en team.ts y ya confirmadas. Si ninguna lo
     está, la fila entera no se pinta. */
        ...
        <article className="panel group relative isolate flex h-full min-w-0 flex-col">
          ...
          <header className="flex items-center justify-between gap-3 border-b border-slate px-4 py-2.5 sm:px-5">
            <span className="flex min-w-0 items-center gap-2">
              ...
              />
              <span className="telemetry-label truncate text-[0.6875rem] sm:text-[0.6875rem]">
                {t.common.brand.name}
              </span>
            </span>
          ...
          <div className="grid grid-cols-[88px_1fr] gap-4 p-4 sm:grid-cols-[104px_1fr] sm:gap-5 sm:p-5 xl:grid-cols-[120px_1fr]">
            <div className="min-w-0">
              ...
              <TiltDepth depth={30} className="relative">
                {portrait && portraitReady ? (
                  ...
                  /* Sin foto todavía: las iniciales, en vez del placeholder de
                     RallyImage, que a este tamaño pintaba la frase entera del
                     `alt` ("Retrato de…, con el casco bajo el brazo"). */
                  <div
                ...
                >
                  <span
                    className={`font-mono text-[0.6875rem] uppercase leading-none tracking-[0.18em] ${tone.accentText}`}
                  >
                    {t.common.brand.rally}
                  </span>
                  <span className="font-mono text-[0.6875rem] uppercase leading-none tracking-[0.12em] text-text-secondary">
            ...
            <div className="flex min-w-0 flex-col">
              <span
                className={`telemetry-label text-[0.6875rem] sm:text-[0.6875rem] ${tone.accentText}`}
              >
                {roleLabel}
              </span>
              ...
              <dl className="mt-4 flex flex-col">
                ...
                >
                  ...
                  {/* Sin `nowrap`: en una tarjeta estrecha la coordenada parte
                      ...
                      desbordar el marco. */}
                  <span className="gps-label mt-0.5 block text-[0.6875rem] leading-snug">
          ...
          {/* ── 3 · Biografía (solo cuando está escrita) ─────────────────── */}
          {!isPending(member.bio) && (
            <div className="border-t border-slate px-4 py-4 sm:px-5">
              <span className="telemetry-label telemetry-label-dash mb-2.5 block text-[0.6875rem]">
          ...
          >
            <p className="whitespace-nowrap font-mono text-[0.6875rem] leading-[1.7] tracking-[0.1em] text-text-tertiary/70 sm:text-[0.6875rem]">
              {mrz[0]}
              <br />
              {mrz[1]}
            </p>
